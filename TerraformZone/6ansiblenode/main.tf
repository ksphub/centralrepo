...
resource "aws_instance" "AnsibleNode" {
  ...
  provisioner "remote-exec" {
    inline = [
      ...
      "echo Successfully Configured Ansible Node"
    ]
    connection {
      type        = "ssh"
      agent       = false
...
output "PrivateIP" {
  value = "${aws_instance.AnsibleNode.private_ip}"
}

output "PublicIP" {
  value = "${aws_instance.AnsibleNode.public_ip}"
}
